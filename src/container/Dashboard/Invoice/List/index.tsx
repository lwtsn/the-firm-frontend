                ...
                <div className={Classes.DIALOG_BODY}>
                    ...
                    <div>
                        ...
                        <div>
                            ...
                            <Button minimal loading={isNewInvoiceCreating} onClick={handleCreateNewInvoiceFormClose}>
                                Close
                            </Button>
                        </div>
                    </div>
                        ...
                        <tbody>
                            {list.map((item) => (
                                <tr key={`invoice-${item.id}`}>
                                    ...
                                    <td>
                                        {item.recipient ? (
                                            <Link to={`/dashboard/recipient/${item.recipient.id}`}>
                                                {item.recipient.name}
                                            </Link>
                                        ) : (
                                            'Custom invoice'
                                    ...
                                    <td>
                                        {item.finalized_at ? format(item.updated_at, 'dd.MM.yyyy') : 'Still pending'}
                                    </td>
                                    <td>
                                    ...
                                    <td className={'fit-content'}>
                                        <Button
                                            onClick={() => history.push(`/dashboard/invoice/${item.id}`)}
            ...
            <div>
                ...
                <div className={'row'}>
                    <div className={'col-xs-12'} style={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <Button icon={'add'} intent={Intent.SUCCESS} onClick={() => handleCreateNewInvoiceFormOpen()}>
                            Create new invoice
                        </Button>
                    </div>
                </div>
